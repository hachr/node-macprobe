...
function probe(opts, callback){
    ...
    opts = opts || {}; 
    var receiver = new Receiver(opts.cacheDuration);
    ...
    receiver.on('discovered', function(device){
        ...
        callback.call(null,device, false);
    });

    receiver.on('updated', function(device){
	//todo: add flag to include oui or not?
	var macKey = device.mac.slice(0,8).replace(/:/g,'');
	var found = oui[macKey];
	if(found){
	...
	callback.call(null,device, true);
    });
    
    //TODO: externalize this cmd
    var pid = spawn('airodump-ng', ['mon0', '--manufacturer','--ignore-negative-one','--berlin', '60', '--probe'], {});
